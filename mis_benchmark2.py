...
def run_all_solvers(inst: MISInstance,
    ...
    for solver_name in ['online_mis', 'redumis']:
        if solver_name in kamis.binaries:
            tl = 10.0 if inst.n <= 100 else 30.0
...
def print_results_table(all_results: Dict[str, List[SolverResult]]):
...
def main():
    ...
    if args.full:
        configs = [
            ...
            ('multi_clique_core', 50, 10, {'q': 3, 'b': 2}),
            ('multi_clique_core', 100, 15, {'q': 3, 'b': 2}),
            ('multi_clique_core', 200, 20, {'q': 3, 'b': 2}),
        ]
    else:
        configs = [
            ('erdos_renyi', 30, 5, {'p': 0.5}),
            ('erdos_renyi', 50, 7, {'p': 0.5}),
            ('multi_clique_core', 30, 8, {'q': 3, 'b': 2}),
            ('multi_clique_core', 50, 10, {'q': 3, 'b': 2}),
        ]
    ...
    for family, n, h, params in configs:
        for seed in [42, 123, 456]:
            ...
            all_results[inst.instance_id] = results
        ...
        json_results[inst_id] = [
            {
                ...
                'recall': r.recall, 'runtime': r.runtime,
